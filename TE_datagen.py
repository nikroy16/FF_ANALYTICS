...
for week in range(1,18):
    parsed = parse(urlopen('http://sports.yahoo.com/nfl/stats/byposition?pos=TE&conference=NFL&year=season_' + str(year) + '&timeframe=Week' + str(week) + '&sort=601&old_category=TE'))
    doc = parsed.getroot()
    ...
    for text in raw_data:
        ...
        if text.replace(" ", "").replace(".", "").replace("'", "").replace("-", "").isalpha() and len(text) > 3:
            ...
            if data.has_key(text) == False:
                data[text] = {"Rec":dict.fromkeys(range(1,18)),"Tgt":dict.fromkeys(range(1,18)), "Rec_Yds":dict.fromkeys(range(1,18)), "Rec_Lng":dict.fromkeys(range(1,18)), "Rec_TD":dict.fromkeys(range(1,18)), "Rush_Att":dict.fromkeys(range(1,18)), "Rush_Yds":dict.fromkeys(range(1,18)), "Rush_TD":dict.fromkeys(range(1,18)), "Fum":dict.fromkeys(range(1,18)), "FumL":dict.fromkeys(range(1,18)) }
